...
async def post_template(template: TemplateRead, response: Response):
    ...
    content, status = await save_object(
        data_to_save, RegistryType.template, user_id=template.userId, account_id=template.accountId
    )
    response.status_code = status
    return content
...
async def put_template(template_id: UUID, template: TemplateRead, response: Response):
    ...
    status = await update_by_id(
        template_id, templ_update, RegistryType.template, account_id=template.accountId, user_id=template.userId
    )
    response.status_code = status
    return
...
async def upload_templates(file_template: UploadFile, response: Response):
    ...
    try:
        ...
        for template in template_list:
            ...
            await save_object(
                TemplateObject.model_validate(template),
