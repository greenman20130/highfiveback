...
async def get_chat(response: Response, company_id: UUID, user_id: UUID,
    ...
    for chats in find_chats:
        ...
        signature = hmac.new(bytearray(COMMENT_TOKEN, 'utf-8'),
                             bytearray(key, 'utf-8'), hashlib.sha1).hexdigest()
        ...
        chat = [message for message in messages if not message["is_deleted"]]
...
async def post_chat(chat: ChatCreateUpdate, response: Response, company_id: UUID, user_id: UUID):
    ...
    if len(find_chat) == 0:
        if data['anonymous'] == False:
            ...
            if len_chats > 0:
                ...
                chat_name_counter = chat_name_counter.replace(
                    'Свободный диалог ', '')
    ...
    key = f'{str(_COMMENT_SERVICE_ID)}{str(company_id)}{str(user_id)}'
    data['signature'] = hmac.new(bytearray(
        COMMENT_TOKEN, 'utf-8'), bytearray(key, 'utf-8'), hashlib.sha1).hexdigest()
