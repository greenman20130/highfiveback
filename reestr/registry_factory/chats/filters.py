...
class ChatFilter(df_filters.FilterSet):
    ...
    object_item = UUIDInFilter(lookup_expr="in")
    account_id = UUIDInFilter(lookup_expr="in")
    user_id = UUIDInFilter(lookup_expr="in")
    project_id = UUIDInFilter(lookup_expr="in")
    object_type = df_filters.BaseInFilter(lookup_expr="in")
    name = df_filters.BaseInFilter(lookup_expr="in")
    object_code = df_filters.BaseInFilter(lookup_expr="in")
    ...
    def meta_internal_id_filter(self, queryset, name, value):
        if name == "meta_internal_id":
            value = value.split(",")
            return queryset.filter(meta__internal_id__in=[int(x) for x in value if x.isnumeric()]).order_by(
                "meta__internal_id"
            )
...
class CustomizedOrdering(rf_filters.OrderingFilter):
    def get_valid_fields(self, queryset, view, context={}):
        ...
        if ordering_params:
            ...
            data_fields = [
                (field[1:], field[1:]) if field.startswith("-") else (field, field)
