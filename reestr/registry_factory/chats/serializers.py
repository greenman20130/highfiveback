...
from .models import Chat, meta_default_value, get_next_internal_id


class ChatsBulkSerializer(serializers.ListSerializer):
    ...
    def create(self, validated_data):
        ...
        a list of all objects to be created and then sends a
        ...
        for item in validated_data:
            ...
            item.pop("meta")
            object_data.append(Chat(meta=meta_data, **item))

        return Chat.objects.bulk_create(object_data)
...
class ChatSerializer(serializers.ModelSerializer):
    id = serializers.UUIDField(validators=[UniqueValidator(queryset=Chat.objects.all())],
                               allow_null=True)
    ...
    def create(self, validated_data):
        ...
        instance = Chat.objects.create(meta=meta_data, **validated_data)
    ...
    class Meta:
        model = Chat
        fields = ['id', 'created_date', 'modified_date', 'project_id', 'account_id',
                  'user_id', 'object_type', 'object_item', 'object_code', 'name',
                  'meta', 'data', ]
        read_only_fields = ['id', 'created_date', 'modified_date']
        list_serializer_class = ChatsBulkSerializer
