...
class LinksViewset(mixins.ListModelMixin, mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def bulk_update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(instances, data=sorted_links_data, partial=False, many=True)
        serializer.is_valid(raise_exception=True)
    ...
    def bulk_partial_update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(instances, data=sorted_links_data, partial=True, many=True)
        serializer.is_valid(raise_exception=True)
        self.perform_update(serializer)
    ...
    def bulk_deactivate(self, request, *args, **kwargs):
        ...
        if len(id_list) == len(objs):
            return Response(data={"message": "Links deactivated"}, status=status.HTTP_200_OK)
        else:
            return Response(
                data={
                    "message": f"{len(objs)} links deactivated out of {len(id_list)}. "
...
class LinkViewset(mixins.RetrieveModelMixin, mixins.CreateModelMixin, mixins.UpdateModelMixin, viewsets.GenericViewSet):
    ...
    def deactivate(self, request, *args, **kwargs):
        ...
        instance.save()
        return Response(data={"message": "Link deactivated"}, status=status.HTTP_200_OK)
